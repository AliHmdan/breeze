...
import 'package:breezefood/features/profile/presentation/ui/address_picker_sheet.dart';
...
class _RequestOrderScreenState extends State<RequestOrderScreen> {
  // ✅ NEW: addresses cubit instance (from getIt)
  late final AddressesCubit _addressesCubit;

  // ✅ address details text (building/floor/etc) - still used
  ...
  void initState() {
    super.initState();
    _addressesCubit = getIt<AddressesCubit>()..load();
  }
  ...
  Widget _recommendedAppetizersSection({
    ...
    if (_appetizersError != null) return const SizedBox.shrink();
    if (_appetizers.isEmpty) return const SizedBox.shrink();
  ...
  Future<void> _storeOrder(
    ...
    final selected = _addressesCubit.selected; // ✅ from API list

    final hasSelected =
        selected != null && selected.latitude != 0 && selected.longitude != 0;

    if (_deliveryType == "delivery" && !hasSelected) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text(
            isRTL ? "اختر عنوان للتوصيل" : "Select a delivery address",
          ),
    ...
    // build address text: base + details
    final details = _tempDetailsCtrl.text.trim();
    final baseText = selected?.address ?? "";
    final finalText = details.isEmpty ? baseText : "$baseText - $details";

    final addressToSend = OrderAddress(
      text: finalText,
                  ...
                  builder: (context, state) {
                    return state.maybeWhen(
                      ...
                      cartLoaded: (cart, updatingIds, toast, isRefreshing) {
                        ...
                        // ✅ selected address -> OrderAddress (for preview)
                        final selected = context
                            .watch<AddressesCubit>()
                            .selected;
                        final previewAddress = (selected == null)
                            ? null
                            : OrderAddress(
                              ...
                              children: [
                                ...
                                if (_deliveryType == "delivery") ...[
                                  AddressSection(
                                    ...
                                    onChangeTap: _openAddressPickerSheet, // ✅
